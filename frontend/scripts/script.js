...
function selectionStep() {
	...
	getTemplateGames('./backend/index.php/services', function() {
		...
		$('.book-button').on('click', function() {
			...
			if($(this).hasClass('enabled')) {
				...
				//var bkcode = "TR";
				//total_cost
				var data = [game, lang, date, hour, players, price.split(" ")[0]*players];
	...
	renderTemplateSecondStep(data, function() {
		...
		$('.reserve').on('click', function() {
			...
			if($(this).hasClass('enabled')) {
				var date = $('#date').text();
				var time = $('#time').text();
				var people = $('#players').text().split(" ")[0];
				var total_cost = $('#total_cost').text().split(" ")[2];
				var game_name = $('#game_name').text().replace(" ", "_");
				var lang_name = $('#lang_name').text();
				...
				var voucher = "";
				var bkcode = "TR";
				if ($("#voucherCode").hasClass( "voucher-success" )) {
...
function checkVoucher() {
	...
	console.log($("#players").text().charAt(0));
	console.log($("#game_name").text().replace(" ", "_"));
	//loading sequnce
	console.log($("#voucherCode").val());
	...
	success:function(data){
		...
		console.log(dataArray.extra_price);
		if (dataArray.message == 200) {
			$("#voucherCode").prop('disabled', true);
			$("#voucherCode").addClass("voucher-success");
		}
		else{
		$("#voucherCode").val(""); //clean intup
		$("#voucherCode").attr("placeholder", getMessageByCode(dataArray.message)); //put message like placeholder
		}	
...
function getMessageByCode(code){
	...
	case 205: 
		...
		break;
	case 207: 
		return('Voucher jeste neni aktnivni');
		break;
	case 222: 
...
//&& $('.langs[style*="visibility: visible"]').find(":selected").val()!="none"
function checkReservationButton() {
	if ($('.time-slot').hasClass('selected') && $('.langs[style*="visibility: visible"]').find(":selected").val()!="none") {
